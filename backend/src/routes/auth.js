...
authApi.get("/status", async (c) => {
    const db = c.env.DB;
    const isEnabledRecord = await db.prepare("SELECT value FROM kv_settings WHERE key = 'ui_lock_enabled'").first();
...
authApi.post("/login", zValidator("json", z.object({ password: z.string() })), async (c) => {
    ...
    let isValid = false;
    if (isUiLockEnabled) {
        const hashRecord = await db.prepare("SELECT value FROM kv_settings WHERE key = 'ui_lock_hash'").first();
...
authApi.post("/refresh", async (c) => {
    ...
    if (tokenRecord.status !== 'active')
        return c.json({ error: "Token is disabled" }, 401);
    if (Math.floor(Date.now() / 1000) > tokenRecord.expires_at)
