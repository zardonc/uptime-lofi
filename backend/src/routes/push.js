...
pushApi.post("/", zValidator("json", batchPayloadSchema, (result, c) => {
    if (!result.success) {
    ...
    const insertStmt = db.prepare(`INSERT INTO raw_metrics (node_id, timestamp, ping_ms, cpu_usage, mem_usage, is_up, containers_json) 
       VALUES (?, ?, ?, ?, ?, ?, ?)`);
    const batchStmts = payload.map((m) => insertStmt.bind(m.node_id, m.timestamp, m.ping ?? null, m.cpu, m.mem, m.is_up ? 1 : 0, m.containers_json ?? null));
    ...
    const updateStmts = Array.from(latestMetrics.values()).map((m) => {
        ...
        return db.prepare(`UPDATE nodes 
         SET status = ?, last_heartbeat = ?, config_json = json_set(COALESCE(config_json, '{}'), '$.agent.containers', ?) 
         WHERE id = ?`).bind(isUpStatus, m.timestamp, m.containers_json ?? null, m.node_id);
    });
    try {
