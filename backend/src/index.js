...
try {
    ...
    if (typeof raw === 'string') {
        allowedCorsOrigins = raw.split(',').map((s) => s.trim()).filter(Boolean);
    }
    else if (Array.isArray(raw)) {
        allowedCorsOrigins = raw;
    }
}
catch {
...
app.use('*', async (c, next) => {
    ...
    const contentLength = c.req.headers?.get?.('content-length') ?? c.req.header?.('content-length');
    if (contentLength) {
