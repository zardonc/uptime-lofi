...
const strictStore = new Map();
const standardStore = new Map();
const permissiveStore = new Map();
// Shared cleanup every 60 seconds
setInterval(() => {
    const now = Date.now();
    const stores = [strictStore, standardStore, permissiveStore];
...
function createLimiter(store, limit, windowMs) {
    return async (c, next) => {
        // Access to request is via c.req; cast to any to avoid DOM lib type issues
        const clientId = extractClientId(c.req);
        const pathKey = getPathKey(c.req);
